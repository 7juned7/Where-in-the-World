const countryData =  [
  ...
  const displayAllCountry = document.querySelector(".world");
  const filterBtn = document.querySelector(".filter-btn")
  const dropdown = document.querySelector(".filter-dropdown");
  const countryCategories = document.querySelectorAll(".options");
...
let i =0;
  ...
  el.classList.add("countries");
  el.innerHTML =`<div class = "countries-card" id="${i}" onclick="call(${i})" >
                ...
                <div class="left-block">
                    Native Name <p>${element.nativeName}</p>
                    Population: <p>${element.population}</p>
                    Region: <p>${element.region}</p>
                    Sub Region: <p>${element.subregion}</p>
                    Capital: <p>${element.capital}</p>
                </div>
                <div class="right-block">
                    Top Level Domain: <p>${element.topLevelDomain}</p>
                    Currencies:${element.currencies}
                    Language: ${element.languages[0].name }
                </div>
                <div class="border-countries">
                
                    Border Countries: ${element.borders}
                </div>
               </div>
               </div>
               </div>
...
function searchCountries(){
  ...
  for(let x in arr){
    if(arr[x].toLowerCase().includes(input)){
     
      const el = document.createElement("div");
      el.classList.add("countries-card");
      el.innerHTML =`<div class="card" onclick="call(${x})" id="${x}">
      <img src="${countryData[x].flags.png}" onclick"call()">
      <div class="country-info">
          <h2>${countryData[x].name}</h2>
      
      <p>Population: ${countryData[x].population}</p>
      <p>Region: ${countryData[x].region}</p>
      <p>Capital:${countryData[x].capital}</p>
      </div>
      </div>`  
      console.log(x)
      console.log(el)
...
let regionId = [];
  element.addEventListener("click",(e)=>{
    ...
    for(let x in countryData){
      if(countryData[x].region == category){
        regionId.push(x);
        const el = document.createElement("div");
        el.classList.add("countries-card");
        el.innerHTML =`<div class="card" onclick="call(${x})">
        <img src="${countryData[x].flags.png}">
        <div class="country-info">
            <h2>${countryData[x].name}</h2>
