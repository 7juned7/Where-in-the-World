...
const displayContent = document.querySelector(".about-country");
...
if(flag==0){
  root.style.setProperty("--Dark-Blue","0, 0%, 100%")
  root.style.setProperty("--Very-Dark-Blue","0, 0%, 97%")
  root.style.setProperty("--White","0, 0%,00%")
  console.log(flag)
  lightModeText.innerHTML= "DARK MODE"
  flag=1;
  
}
else{
  root.style.setProperty("--Dark-Blue","209, 23%, 22%");
  root.style.setProperty("--Very-Dark-Blue","207, 26%, 17%");
  root.style.setProperty("--White","0, 0%, 100%");
  console.log(flag);
  lightModeText.innerHTML= "LIGHT MODE";
  flag=0;
...
el.classList.add("display-country");

el.innerHTML = ` <img src="${countryData[Id].flags.png}" alt="">
<div class="country-info">
    <h2>${countryData[Id].name}</h2>
   <div class="country-details">
    <div class="left-block">
         <p><span>Native Name:</span>${countryData[Id].nativeName}</p>
         <p> <span>Population:</span>${countryData[Id].population}</p>
         <p><span>Region:</span> ${countryData[Id].region}</p>
         <p> <span>Sub Region: </span>${countryData[Id].subregion}</p>
         <p> <span>Capital:</span> ${countryData[Id].capital}</p>
    </div>
    <div class="right-block">
        <p><span>Top Level Domain: </span>${countryData[Id].topLevelDomain}</p>
         <p class="currencies"><span>Currencies:</span></p>
         <p class="languages"><span>Languages:</span></p>
    </div>
    </div>
    <div class="border-countries">
    
        <p class="border-tag">Border Countries:</p>
        <p class="border"></p>
    
   </div>
</div>`
const lang = el.querySelector(".languages");
const curre = el.querySelector(".currencies");
const borderCountry = el.querySelector(".border")

for( let x in countryData[Id].borders){
   
    let borderC = countryData[Id].borders[x];
   
    borderCountry.innerHTML += `<span class="borders">${borderC}</span>`;
}

for( let x in countryData[Id].currencies){
    let curr = countryData[Id].currencies[x].name;
    // const curr = countryData[Id].currencies[x].name
  
   curre.innerHTML+= curr;
}
for( let x in countryData[Id].languages){
    const langs = (countryData[Id].languages[x].name)
  
   lang.innerHTML+= langs+ " ";
}
displayContent.append(el);
